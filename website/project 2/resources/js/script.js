...
function refreshLightBri() {
    lampen.forEach(async (lamp) => {
        ...
        lamp.bri = bri;
        const hueCardBrightness = document.getElementById("hueCardBrightness" + lamp.id);
...
function RefreshLightsState() {
    lampen.forEach(async (lamp) => {
        ...
        hueCardState.style.color = lamp.state
...
async function placeShellyOnDom() {
    ...
    while (document.getElementById("shellyContainer").firstChild) {
        document
            .getElementById("shellyContainer")
            .removeChild(document.getElementById("shellyContainer").firstChild);
    }
    ...
    shellyPlugValue.forEach((key) => {
        ...
        if (shellyPlug._dev_info.code == "SHPLG-S") {
            const shellyPlugIp = shellyPlug.wifi_sta.ip;
...
async function placeHueOnDom() {
    ...
    while (document.getElementById("hueContainer").firstChild) {
        document
            .getElementById("hueContainer")
            .removeChild(document.getElementById("hueContainer").firstChild);
    }
...
async function RefreshShellyInfo() {
    if (plugs) {
        plugs.forEach(async (plug) => {
            ...
            plug.power = power;
            document.getElementById("shellypower" + plug.id).textContent =
                power + "W";
        });
    }
}
function createShellyCard(plug) {
    ...
    shellyCardState.style.color = plug.state
        ? "var(--icon-active-color)"
        : "var(--icon-inactive-color)";
...
function createHueCard(plug) {
    ...
    hueCardState.style.color = plug.state
        ? "var(--icon-active-color)"
        : "var(--icon-inactive-color)";
